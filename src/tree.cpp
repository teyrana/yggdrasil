...
WorldTree::WorldTree(const Layout& _layout)
    : layout(_layout) 
{ 
    reset();
}
...
size_t WorldTree::get_memory_usage() const {
...
void WorldTree::fill(const cell_value_t fill_value){
    root->fill(fill_value);
}

size_t WorldTree::get_height() const {
...
void WorldTree::prune(){
    root->prune();
}

void WorldTree::reset(){
...
size_t WorldTree::size() const {
    return root->get_count();
}

json WorldTree::to_json_tree() const {
    return root->to_json();
}
