...
template <typename T> Terrain<T>::Terrain() : Terrain(*(new T())) {}

template <typename T> Terrain<T>::Terrain(T& _ref) : impl(_ref) {
#ifdef ENABLE_GDAL
    GDALAllRegister();
#endif
}
...
template <typename T> void Terrain<T>::debug() const {
    ...
    cerr << "             ";
    for (double x = (layout.get_x_min() + precision / 2);
         x < layout.get_x_max(); x += precision) {
    ...
    for (double y = (layout.get_y_max() - precision / 2);
         y > layout.get_y_min(); y -= precision) {
        // print left header:
...
template <typename T> void inline Terrain<T>::fill(const cell_value_t value) {
    impl.fill(value);
}

template <typename T>
void inline Terrain<T>::fill(const Polygon& poly,
                             const cell_value_t fill_value) {
    // adapted from:
    ...
    for (double y = layout.get_y_min() + precision_2; y < layout.get_y_max();
         y += precision) {
        ...
        for (int crossing_index = 0; crossing_index < crossings.size();
            ...
            const double start_x =
                layout.constrain_x(crossings[crossing_index] + precision_2);
            const double end_x =
                layout.constrain_x(crossings[crossing_index + 1] + precision_2);
...
template <typename T> const Layout& Terrain<T>::get_layout() const {
    return impl.get_layout();
}

template <typename T> const string& Terrain<T>::get_error() const {
    return error_message;
}

template <typename T> inline double Terrain<T>::get_load_factor() const {
...
template <typename T> void inline Terrain<T>::reset() { impl.reset(); }

template <typename T> void inline Terrain<T>::reset(const Layout& _layout) {
    impl.reset(_layout);
}

template <typename T> std::string Terrain<T>::summary() const {
    std::ostringstream buffer;
    buffer.imbue(std::locale(""));
    buffer << "====== Terrain Stats: ======\n";
    buffer << "##  layout:       " << impl.get_layout().to_string() << '\n';
    buffer << "##  dimension:    " << impl.get_layout().get_dimension() << endl;
    buffer << "##  size:         " << impl.get_layout().get_size()
           << " nodes  ===  " << impl.get_memory_usage() / 1000
           << " kilobytes\n";
