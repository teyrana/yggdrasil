...
ZOrderCurveIndex<dimension, cell_t, storage_t>::ZOrderCurveIndex(
    storage_t& _data)
    : data(_data) {}

template <size_t dimension, typename cell_t, typename storage_t>
constexpr cell_t
ZOrderCurveIndex<dimension, cell_t, storage_t>::lookup(const uint32_t i,
                                                       const uint32_t j) {
...
template <size_t dimension, typename cell_t, typename storage_t>
constexpr cell_t&
ZOrderCurveIndex<dimension, cell_t, storage_t>::operator()(const uint32_t i,
                                                           const uint32_t j) {
    const uint64_t index = _interleave(i) + (_interleave(j) << 1);

    // const uint64_t raw_index = interleave(i) + (interleave(j) << 1);
    // const uint64_t index = raw_index << ZOrderCurveIndex<dimension,
    // cell_t>::padding;
...
template <size_t dimension, typename cell_t, typename storage_t>
constexpr const cell_t&
ZOrderCurveIndex<dimension, cell_t, storage_t>::operator()(
