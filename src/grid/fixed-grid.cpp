...
Index2u FixedGrid<cell_t,dim>::as_index(const Eigen::Vector2d& location) const {
...
template<typename cell_t, size_t dim>
Vector2d  FixedGrid<cell_t,dim>::as_location(const index::Index2u& index) const {
...
template<typename cell_t, size_t dim>
bool FixedGrid<cell_t,dim>::blocked(const index::Index2u& at) const {
...
template<typename cell_t, size_t dim>
bool FixedGrid<cell_t,dim>::contains(const Index2u& index) const {
...
template <typename cell_t, size_t dim>
int FixedGrid<cell_t, dim>::fill_from_buffer(const std::vector<cell_t>& source) {
...
template<typename cell_t, size_t dim>
cell_t& FixedGrid<cell_t,dim>::operator[](const index::Index2u& location) {
    return grid[index.lookup(location)];
}

template<typename cell_t, size_t dim>
cell_t FixedGrid<cell_t,dim>::operator[](const index::Index2u& location) const {
    return grid[index.lookup(location)];
}
