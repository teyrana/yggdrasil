...
class WorldTree {
  public:
     ...
     *
     * Use this constructor if the tree will be loaded from a config file, and
     * the initialization values do not matter.
     */
    WorldTree();
     ...
     * @param {double} x The x-coordinate.
     * @param {double} y The y-coordinate.
     * @param {cell_value_t} opt_default The default value to return if the node
     * doesn't exist.
     * @return {cell_value_t} The value of the node, if available; or the
     * default value.
     */
    ...
    /**
     * Gets the value of the point at (x, y).  If the point is not close to the
     * center of a node, this function interpolates or extrapolates an
     * appropriate value.
     *
     * @param {double} x The x-coordinate.
     * @param {double} y The y-coordinate.
    ...
    /**
     * Loads a representation of a tree from the data source.  The the form
     * source is assumed to contain a serialization of a tree, as represented in
     * valid json.  That is, in the same format as Tree::serialize(...).
     *
    ...
    ///! \brief resets _the tree_ to fully populate the bounds at the given
    /// precision
    ///!
